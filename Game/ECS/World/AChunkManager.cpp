...
void AChunkManager::LoadChunksAround(const glm::vec3& centerPos, int horizontalRadius, int verticalRadius) {
    {
        ...
        for (int dz = -m_renderRadius; dz <= m_renderRadius; ++dz) {
            for (int dx = -m_renderRadius; dx <= m_renderRadius; ++dx) {
                desired.emplace(ChunkKey{ glm::ivec3(centerPos.x + dx, 0, centerPos.z + dz) });
            }
        }
        ...
        for (const auto &k : toLoad) {
            LOG_INFO("Chunks", "Loading chunk at ({}, {})", k.coord.x, k.coord.y);
            LoadChunk(k.coord);

        }

        for (const auto &k : toUnload) {
            LOG_INFO("Chunks", "Unloading chunk at ({}, {})", k.coord.x, k.coord.y);
            UnloadChunk(k.coord);
        }

        LOG_INFO("Chunks", "After LoadChunksAround center=({}, {}) loaded_count={}",
                 centerPos.x, centerPos.z, m_loadedChunks.size());
    }
}
...
void AChunkManager::Update(float deltaTime, const glm::vec3& playerPos) {
    ...
    if (centerChunk != m_lastCenterChunk) {
        ...
        LoadChunksAround(glm::vec3(centerChunk.x, centerChunk.y, centerChunk.z), 3, 1);
    }
}
